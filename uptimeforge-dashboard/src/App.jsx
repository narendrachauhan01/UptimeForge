...
const UdpMonitorPage    = lazy(() => import('./pages/UdpMonitorPage'));
...
const IcoUdp      = () => <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.68 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/><path d="M10 10l-2 2 2 2M14 10l2 2-2 2"/></svg>;
...
function AppInner() {
  ...
  const [freeAccess, setFreeAccess] = useState({ domainSsl: false, charts: false, pingMonitor: false, pingMonitorIcmp: false, dnsMonitor: false, udpMonitor: false, whatsapp: false, telegram: false, webhook: false, rocketChat: false, slack: false, reports: false });
  const [bronzeAccess, setBronzeAccess] = useState({ pingMonitor: false, pingMonitorIcmp: false, dnsMonitor: false, udpMonitor: false, whatsapp: false, telegram: false, webhook: false, rocketChat: false, slack: false, reports: false });
  const [silverAccess, setSilverAccess] = useState({ pingMonitor: false, pingMonitorIcmp: false, dnsMonitor: false, udpMonitor: false, whatsapp: false, telegram: false, webhook: false, rocketChat: false, slack: false, reports: false });
  const [goldAccess,   setGoldAccess]   = useState({ pingMonitor: false, pingMonitorIcmp: false, dnsMonitor: false, udpMonitor: false, whatsapp: false, telegram: false, webhook: false, rocketChat: false, slack: false, reports: false });
  const navigate = useNavigate();
  const location = useLocation();
        ...
        <main className="content">
          <Suspense fallback={null}>
            <Routes>
              ...
              <Route path="/udp" element={(() => {
                  const planAcc = { free_trial: freeAccess, bronze: bronzeAccess, silver: silverAccess, gold: goldAccess };
                  const acc = user?.plan ? planAcc[user.plan] : null;
                  const blocked = acc && acc.udpMonitor === false;
                  return blocked ? <UpgradeGate user={user} feature="UDP Monitoring" /> : <UdpMonitorPage />;
              })()} />
              <Route path="/terms" element={<TermsOfService />} />
