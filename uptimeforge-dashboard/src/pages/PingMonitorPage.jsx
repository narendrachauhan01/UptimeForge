let _loaded_IcmpPingMonitor = false;
...
function TargetModal({ target, onClose, onSave }) {
    const [form,         setForm]         = useState(target || { name:'', host:'' });
    ...
    const save = async () => {
        ...
        setSaveError('');
        try {
            const payload = { name: form.name, host: form.host, notifyRecipients: selected };
            ...
            <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-color)', borderRadius:20, width:'100%', maxWidth:420, padding:28, boxShadow:'var(--card-shadow)', position:'relative' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    ...
                    <div>
                        ...
                        <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:4 }}>Works for any device that responds to ping — servers, routers, printers, IoT devices.</div>
                    </div>
                </div>
                {/* Recipients */}
...
function DetailModal({ target, onClose, onDelete, onToggle, onEdit }) {
    ...
    const doPing = useCallback(async () => {
        ...
        try {
            const r = await axios.post(`${API_URL}/api/ping/icmp`, { target:target.host }, { withCredentials: true });
            r.data.alive
    ...
    const start = async () => {
        setRunning(true);
        addLine(`PING ${target.host}`, '#60a5fa');
                ...
                <div style={{ background:'var(--bg-input)', padding:'18px 22px', borderBottom:'1px solid var(--border-color)', borderRadius:'20px 20px 0 0', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize:12, color:'var(--text-muted)', fontFamily:'monospace' }}>{target.host}</div>
                        </div>
                    </div>
                ...
                <div style={{ padding:18, display:'flex', flexDirection:'column', gap:14 }}>
                        ...
                        {chartData.length > 1 ? (
                            <ResponsiveContainer width="100%" height={150}>
                                <AreaChart data={chartData} margin={{top:5,right:10,left:8,bottom:0}}>
                                    <defs><linearGradient id="pgGrad2" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#7c3aed" stopOpacity={0.2}/><stop offset="95%" stopColor="#7c3aed" stopOpacity={0}/></linearGradient></defs>
                                    ...
                                    <Area type="monotone" dataKey="ms" stroke="var(--primary)" strokeWidth={2.5} fill="url(#pgGrad2)" dot={false}/>
                                </AreaChart>
                            </ResponsiveContainer>
                    ...
                    <div style={{ background:'#0d1117', borderRadius:14, overflow:'hidden', border:'1px solid #30363d' }}>
                        <div style={{ background:'#161b22', padding:'9px 14px', display:'flex', alignItems:'center', gap:10 }}>
                            ...
                            <span style={{ flex:1, fontFamily:'monospace', fontSize:12, color:'#8b949e' }}>ping {target.host}</span>
...
export default function PingMonitorPage() {
    ...
    const [pageLoading, setPageLoading] = useState(!_loaded_IcmpPingMonitor);
    ...
    const load = () =>
        axios.get(`${API_URL}/api/icmp-targets`, { withCredentials: true })
            .then(r=>{ setTargets(r.data); setPageLoading(false); _loaded_IcmpPingMonitor = true; })
    ...
    const addTarget = async (form) => {
        await axios.post(`${API_URL}/api/icmp-targets`, form, { withCredentials: true });
        load();
    };
    const editTargetSave = async (form) => {
        const r = await axios.put(`${API_URL}/api/icmp-targets/${editTarget._id}`, form, { withCredentials: true });
        setEditTarget(null);
        load();
        return r;
    };
    const deleteTarget = async (id) => {
        ...
        await axios.delete(`${API_URL}/api/icmp-targets/${id}`, { withCredentials: true });
        setSelected(null); load();
    };
    const toggleTarget = async (t) => {
        await axios.put(`${API_URL}/api/icmp-targets/${t._id}`, { active:!t.active }, { withCredentials: true });
        load();
    };
    ...
    return (
      <div className={`perf-page-container ${localTheme}`}>
        <style>{`
          ...
          .perf-bg-glow-1 {
            position: absolute; top: -200px; right: 10%; width: 600px; height: 600px;
          ...
          .perf-bg-glow-2 {
            position: absolute; bottom: -150px; left: -50px; width: 500px; height: 500px;
          ...
          .perf-page-container .mon-panel {
            width: 280px; flex-shrink: 0; background: var(--bg-card) !important;
            border-left: 1px solid var(--border-color) !important; border-radius: 24px; padding: 24px;
            display: flex; flex-direction: column; gap: 0; box-shadow: var(--card-shadow);
            height: fit-content; box-sizing: border-box; transition: all 0.3s;
          }
          ...
          .perf-page-container .mon-btn-check {
            padding: 8px 18px; background: linear-gradient(135deg, #7c3aed, #6d28d9) !important; color: #fff !important;
            border: none !important; border-radius: 12px !important; font-size: 13px; font-weight: 700; cursor: pointer;
            transition: all 0.2s; box-shadow: 0 4px 12px rgba(124,58,237,0.2) !important;
          }
          ...
          .perf-page-container .mon-search {
            width: 100%; padding: 9px 12px 9px 36px; border: 1.5px solid var(--border-color) !important; border-radius: 12px;
            background: var(--bg-input) !important; color: var(--text-main) !important; font-size: 13px; font-weight: 600;
            outline: none; box-sizing: border-box; font-family: 'Plus Jakarta Sans', sans-serif; transition: all 0.2s;
          }
          ...
          .perf-page-container .mon-row {
            ...
            background: var(--bg-card) !important; border: 1px solid var(--border-color) !important; cursor: pointer;
            transition: all 0.25s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: var(--card-shadow);
          }
