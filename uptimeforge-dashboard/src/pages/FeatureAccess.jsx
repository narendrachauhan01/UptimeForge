...
const FEATURES = [
    ...
    { key: 'infraMonitor',   label: 'Infra Monitor',            desc: 'Install agent on server to monitor CPU, RAM, Disk, SSH sessions in real-time', icon: <span style={{fontSize:20}}>🖥️</span> },
];
...
export default function FeatureAccess({ readOnly = false }) {
    const DEFAULT_ACC = { pingMonitor: false, pingMonitorIcmp: false, dnsMonitor: false, udpMonitor: false, apiMonitor: false, whatsapp: false, telegram: false, webhook: false, rocketChat: false, slack: false, discord: false, reports: false, infraMonitor: false };
                ...
                <div className="form-card" style={{ marginTop: 16 }}>
                    ...
                    <div style={{ padding: '16px 20px' }}>
                        <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 10 }}>
                            Backend URL used in agent install commands. Update if domain changes.
                        </p>
                        <input
                            type="url"
                            value={agentApiUrl}
                            ...
                            placeholder="https://uptimeapi.yourdomain.com"
                            style={{ width: '100%', padding: '10px 14px', borderRadius: 8, border: '1px solid var(--border-color)', background: 'var(--bg-input)', color: 'var(--text-main)', fontSize: 14, boxSizing: 'border-box' }}
                        />
                    </div>
                </div>
