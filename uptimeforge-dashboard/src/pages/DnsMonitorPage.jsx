...
function TargetModal({ target, onClose, onSave }) {
    const [form,         setForm]         = useState(target || { name:'', hostname:'', recordType:'A', expectedValue:'', dnsServer:'' });
    ...
    const save = async () => {
        ...
        setSaveError('');
        try {
            const payload = { name: form.name, hostname: form.hostname, recordType: form.recordType, expectedValue: form.expectedValue || '', dnsServer: form.dnsServer || '', notifyRecipients: selected };
            ...
            <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-color)', borderRadius:20, width:'100%', maxWidth:440, padding:28, boxShadow:'var(--card-shadow)', position:'relative', maxHeight:'90vh', overflowY:'auto' }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    <div>
                        ...
                        <input value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Company Website DNS"
                    ...
                    <div>
                        <label style={{ fontSize:12, fontWeight:700, color:'var(--text-main)', display:'block', marginBottom:6 }}>Hostname / Domain *</label>
                        <input value={form.hostname} onChange={e=>setForm({...form,hostname:e.target.value})} placeholder="example.com"
                    ...
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                        ...
                        <div>
                            ...
                            <input value={form.dnsServer} onChange={e=>setForm({...form,dnsServer:e.target.value})} placeholder="Blank = default"
                                style={{ width:'100%', padding:'10px 14px', border:'1.5px solid var(--border-color)', borderRadius:9, fontSize:14, background:'var(--bg-input)', color:'var(--text-main)', outline:'none', boxSizing:'border-box' }} />
                        </div>
                    </div>
                    <div>
                        <label style={{ fontSize:12, fontWeight:700, color:'var(--text-main)', display:'block', marginBottom:6 }}>Expected Value</label>
                        <input value={form.expectedValue} onChange={e=>setForm({...form,expectedValue:e.target.value})} placeholder="e.g. 203.0.113.10 (blank = just verify it resolves)"
                            style={{ width:'100%', padding:'10px 14px', border:'1.5px solid var(--border-color)', borderRadius:9, fontSize:14, background:'var(--bg-input)', color:'var(--text-main)', outline:'none', boxSizing:'border-box' }} />
                        <div style={{ fontSize:11, color:'var(--text-muted)', marginTop:4 }}>If the resolved record doesn't contain this value, the target is marked DOWN.</div>
                    </div>
                </div>
                {/* Recipients */}
                ...
                <div style={{ display:'flex', gap:10, marginTop:16 }}>
                    ...
                    <button onClick={save} disabled={saving || !form.name || !form.hostname}
                        style={{ flex:2, padding:'11px', border:'none', borderRadius:10, background:'linear-gradient(135deg,#7c3aed,#6d28d9)', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', opacity:(saving||!form.name||!form.hostname)?0.6:1, boxShadow:'0 2px 8px rgba(124,58,237,0.2)' }}>
...
function DetailModal({ target, onClose, onDelete, onToggle, onEdit }) {
    ...
    const resolveNow = async () => {
        setResolving(true);
        addLine(`dig ${target.hostname} ${target.recordType}${target.dnsServer ? ' @' + target.dnsServer : ''}`, '#60a5fa');
        addLine('─'.repeat(42), '#1e2d3d');
        try {
            const r = await axios.post(`${API_URL}/api/dns/resolve`, { hostname: target.hostname, recordType: target.recordType, dnsServer: target.dnsServer }, { withCredentials: true });
            if (r.data.resolved) {
                r.data.values.forEach(v => addLine(`${target.hostname}.  IN  ${target.recordType}  ${v}`, '#4ade80'));
                addLine(`;; Query time: ${r.data.ms} msec`, '#94a3b8');
            } else {
                addLine(`;; No answer (${r.data.error || 'resolution failed'})`, '#f87171');
            }
        } catch { addLine(`Error resolving ${target.hostname}`, '#f87171'); }
        setResolving(false);
    };
            ...
            <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-color)', borderRadius:20, width:'100%', maxWidth:800, maxHeight:'94vh', overflowY:'auto', boxShadow:'var(--card-shadow)' }}>
                ...
                <div style={{ background:'var(--bg-input)', padding:'18px 22px', borderBottom:'1px solid var(--border-color)', borderRadius:'20px 20px 0 0', display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12 }}>
                        ...
                        <div>
                            ...
                            <div style={{ fontSize:12, color:'var(--text-muted)', fontFamily:'monospace' }}>{target.hostname} · {target.recordType}</div>
                        </div>
                    </div>
                ...
                <div style={{ padding:18, display:'flex', flexDirection:'column', gap:14 }}>
                    <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:10 }}>
                        {[
                            { l:'Status',   v: target.status==='up'?'Resolving':target.status==='down'?'Failing':'Unknown', c:statColor },
                    ...
                    <div style={{ background:'var(--bg-input)', borderRadius:14, border:'1px solid var(--border-color)', padding:'14px 16px', display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
                        <div>
                            <div style={{ fontSize:11, color:'var(--text-muted)', fontWeight:700, textTransform:'uppercase', marginBottom:6 }}>Expected Value</div>
                            <div style={{ fontFamily:'monospace', fontSize:13, color:'var(--text-main)' }}>{target.expectedValue || <span style={{color:'var(--text-muted)'}}>Any (just verify it resolves)</span>}</div>
                        </div>
                        <div>
                            <div style={{ fontSize:11, color:'var(--text-muted)', fontWeight:700, textTransform:'uppercase', marginBottom:6 }}>Last Resolved</div>
                            <div style={{ fontFamily:'monospace', fontSize:13, color:'var(--text-main)' }}>{target.resolvedValues?.length ? target.resolvedValues.join(', ') : <span style={{color:'var(--text-muted)'}}>No answer</span>}</div>
                        </div>
                    </div>
                        ...
                        {chartData.length > 1 ? (
                            <ResponsiveContainer width="100%" height={150}>
                                <AreaChart data={chartData} margin={{top:5,right:10,left:8,bottom:0}}>
                                    <defs><linearGradient id="dnsGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#7c3aed" stopOpacity={0.2}/><stop offset="95%" stopColor="#7c3aed" stopOpacity={0}/></linearGradient></defs>
                                    ...
                                    <Area type="monotone" dataKey="ms" stroke="var(--primary)" strokeWidth={2.5} fill="url(#dnsGrad)" dot={false}/>
                                </AreaChart>
                            </ResponsiveContainer>
                    ...
                    <div style={{ background:'#0d1117', borderRadius:14, overflow:'hidden', border:'1px solid #30363d' }}>
                        <div style={{ background:'#161b22', padding:'9px 14px', display:'flex', alignItems:'center', gap:10 }}>
                            ...
                            <span style={{ flex:1, fontFamily:'monospace', fontSize:12, color:'#8b949e' }}>dig {target.hostname} {target.recordType}</span>
                            <div style={{ display:'flex', gap:6 }}>
                                ...
                                <button onClick={resolveNow} disabled={resolving} style={{ padding:'3px 12px', background:'#238636', border:'none', borderRadius:6, color:'#fff', fontSize:11, fontWeight:700, cursor:'pointer', opacity:resolving?0.6:1 }}>
                                    {resolving ? '...' : '▶ Resolve Now'}
                                </button>
                            </div>
                        </div>
                        <div ref={termRef} style={{ height:180, overflowY:'auto', padding:'10px 14px', fontFamily:'monospace', fontSize:12, lineHeight:1.75 }}>
                            {lines.length===0 && <div style={{ color:'#4d5566' }}>Press ▶ Resolve Now to query DNS live...</div>}
                    ...
                    <div style={{ background:'var(--primary-glow)', border:'1px solid var(--border-color)', borderRadius:12, padding:'12px 16px', fontSize:13, color:'var(--text-muted)', display:'flex', alignItems:'center', gap:10 }}>
                        ...
                        <div>
                            <strong style={{ color:'var(--primary)' }}>Alerts active</strong> — When this target fails to resolve or recovers, selected recipients will be notified via Email &amp; WhatsApp. Webhooks also fire automatically.
                        </div>
                    </div>
...
export default function DnsMonitorPage() {
    ...
    const [pageLoading, setPageLoading] = useState(!_loaded_DnsMonitor);
    ...
    const load = () =>
        axios.get(`${API_URL}/api/dns-targets`, { withCredentials: true })
            .then(r=>{ setTargets(r.data); setPageLoading(false); _loaded_DnsMonitor = true; })
    ...
    const addTarget = async (form) => {
        await axios.post(`${API_URL}/api/dns-targets`, form, { withCredentials: true });
        load();
    };
    const editTargetSave = async (form) => {
        const r = await axios.put(`${API_URL}/api/dns-targets/${editTarget._id}`, form, { withCredentials: true });
        setEditTarget(null);
        load();
        return r;
    };
    const deleteTarget = async (id) => {
        ...
        await axios.delete(`${API_URL}/api/dns-targets/${id}`, { withCredentials: true });
        setSelected(null); load();
    };
    const toggleTarget = async (t) => {
        await axios.put(`${API_URL}/api/dns-targets/${t._id}`, { active:!t.active }, { withCredentials: true });
        load();
    };

    const filtered = targets.filter(t => !search || t.name.toLowerCase().includes(search.toLowerCase()) || t.hostname.includes(search));
        ...
        <div className="mon-layout" style={{ padding: '0 4px' }}>
            <div className="mon-main">
                <div className="mon-header">
                    <div>
                        <h1 className="mon-title">DNS Monitoring <span className="mon-dot">.</span></h1>
                        <p className="mon-sub">Monitor DNS servers and verify that DNS records resolve to expected values.</p>
                    </div>
                    <button onClick={() => {
                        if (pingLimit !== null && targets.length >= pingLimit) {
                            setLimitError(`DNS target limit reached (${pingLimit}/${pingLimit}). Upgrade your plan to add more.`);
                            return;
                        }
                        setLimitError('');
                        setAddModal(true);
                ...
                <div className="mon-toolbar">
                    ...
                    <div className="mon-filter-tabs">
                        {[['all','All',targets.length],['up','Resolving',up],['down','Failing',down]].map(([v,l,c])=>(
                        ...
                        <div className="mon-empty">
                            <div style={{width:64,height:64,borderRadius:16,background:'rgba(34,197,94,0.12)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 14px'}}>
                                <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                                    ...
                                    <circle cx="6.5" cy="6" r="0.6" fill="#22c55e" stroke="none"/>
                                    <circle cx="6.5" cy="12" r="0.6" fill="#22c55e" stroke="none"/>
                                    <circle cx="6.5" cy="18" r="0.6" fill="#22c55e" stroke="none"/>
                                </svg>
                            </div>
                        ...
                        <div key={t._id} className={`mon-row mon-row-${t.status}`} onClick={()=>setSelected(t)}>
                            ...
                            <div className="mon-site-info">
                                ...
                                <div className="mon-site-meta">
                                    <span className="mon-proto">{t.recordType}</span>
                                    <span className="mon-sep">·</span>
                                    <span className={`mon-status-txt mon-status-${t.status}`}>
                                        {t.status==='up'?'Resolving':t.status==='down'?'Failing':'Unknown'}
                                    </span>
                                    <span className="mon-sep">·</span>
                                    <span className="mon-time" style={{fontFamily:'monospace',fontSize:11}}>{t.hostname}</span>
                                    {t.responseTime && <>
                                        <span className="mon-sep">·</span>
            ...
            <div className="mon-panel">
                <div className="mon-panel-section">
                    ...
                    <div className="mon-panel-counts">
                        ...
                        <div className="mon-count-item mon-count-up"><div className="mon-count-num">{up}</div><div className="mon-count-label">Resolving</div></div>
                ...
                <div className="mon-panel-section">
                    ...
                    {[{l:'Resolving',c:up,color:'#10b981'},{l:'Failing',c:down,color:'#ef4444'}].map(item=>(
                        <div key={item.l} style={{display:'flex',alignItems:'center',gap:10,marginBottom:8}}>
                            ...
                            <span style={{fontSize:11,color:'var(--text-muted)',minWidth:60}}>{item.l}</span>
                        </div>
                    ))}
                </div>
            </div>
