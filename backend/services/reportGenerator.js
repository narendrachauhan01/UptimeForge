...
function generateHTML(data) {
    ...
    const sslSection  = sslWarnings.length ? `
        <div class="ssl-box">
            ...
            ${sslWarnings.map(m => `<div class="ssl-row">&bull; <strong>${esc(m.name)}</strong> — expires in <span style="color:#dc2626;font-weight:700">${m.sslDaysLeft} days</span> &nbsp; ${esc(m.url)}</div>`).join('')}
        </div>` : '';
...
<link href="https://fonts.googleapis.com/css2?family=Inter:ital,opsz,wght@0,14..32,400;0,14..32,500;0,14..32,600;0,14..32,700;0,14..32,800;0,14..32,900&family=JetBrains+Mono:wght@500;600&display=swap" rel="stylesheet">
<style>
...
.pdf-btn:hover { background: #4338ca; box-shadow: 0 6px 18px rgba(79,70,229,.4); }
...
.rpt-title { font-size: 18px; font-weight: 800; color: #f8fafc; letter-spacing: -0.3px; line-height: 1.2; }
...
@media print {
  ...
  tr.alt td { background: #fff8f8 !important; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
}
</style>
</head>
<body>
